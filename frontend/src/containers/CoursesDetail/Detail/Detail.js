...
const Detail = (props) => {
  ...
  const quanRate = [
    props.categories.Rate1,
    props.categories.Rate2,
    props.categories.Rate3,
    props.categories.Rate4,
    props.categories.Rate5,
  ];
  ...
  function handleClickNumberNo(product) {
    if (props.categories.IsRes == 0) {
      ...
      if (productView.indexOf(product.ProductId) < 0) {
        CoursesServices()
          .updateViewVideo(product.ProductId, product.Viewer + 1)
          .then(() => {
    ...
    <div>
      ...
      <div className="courses-post">
        <div className="ttr-post-media media-effect">
          ...
          <span>
            {" "}
            Cập nhật: {moment(props.categories.Update_At).format("DD/MM/YYYY")}
          </span>
        </div>
        <div className="ttr-post-info">
          ...
          <div className="ttr-post-text">
            <p style={{ textAlign: "justif" }}>
              {" "}
              {ReactHtmlParser(props.categories.Note)}
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="ttr-sidebar-navi">
              ...
              >
                <span className="ttr-icon">
                  ...
                  ) : props.categories.IsRes ? (
                    <i className="fa fa-book" />
                  ) : (
                    <i className="fa fa-lock" />
                  )}{" "}
                </span>
                <span className="ttr-label">{`Bài ${
